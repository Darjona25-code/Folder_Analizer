...
| `archive/benchmarks/` | Fixture generator + smoke/export benchmarks (fixed baseline). Phase 8: `--affinity HEX` fixed-mask pinning (reproducible), KB µs/file measurement, `--cancel` cancellation-responsive probe. Relocated from `benchmarks/` on 2026-09-18 (non-active dev tooling) | Tooling |
